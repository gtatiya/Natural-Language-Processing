...
class FSA:
    ...
    def add_trasition(self, input_symbol, state, next_state):
        self.transitions[(input_symbol, state)] = next_state

    def add_trasition_List(self, input_list, state, next_state):
        for i in input_list:
            self.add_trasition(i, state, next_state)

    def set_final_state(self, state):
        self.final_states.add(state)

    def find_next_state(self, input_symbol, state):
        return self.transitions[(input_symbol, state)]

    def check_sinal_state(self, state):
...
def NDRecognize(input, fsa):
    ...
    while True:
        if accept_state(input, fsa, current_state, index):
            return True
        else:
            agenda.extend(generate_new_states(input[index], current_state, index, fsa))
        if len(agenda) == 0:
            return False
        else:
            current_state, index = agenda.pop()
...
def generate_new_states(input_symbol, current_state, index, fsa):
    if (input_symbol, current_state) in fsa.transitions:
        pot_next = fsa.find_next_state(input_symbol, current_state)
        pot_idx = index + 1
        result = [(pot_next, pot_idx)]
    else:
        result = []
    if ('eps', current_state) in fsa.transitions:
        result.append((fsa.find_next_state('eps', current_state), index))
    return result


def accept_state(input, fsa, current_state, index):
    if index == len(input) and fsa.check_sinal_state(current_state):
        return True
...
days.add_trasition("0", 0, 1)
days.add_trasition_List(["1", "2"], 0, 2)
days.add_trasition("3", 0, 3)
...
years.add_trasition("1", 0, 1)
years.add_trasition("2", 0, 2)
years.add_trasition("9", 1, 3)
years.add_trasition("0", 2, 3)
years.add_trasition_List(list2, 3, 4)
years.add_trasition_List(list2, 4, 5)

""" Build FSA for Separators """
seps = FSA(1)
seps.set_final_state(1)
seps.add_trasition_List(["/", " ", "-"], 0, 1)
